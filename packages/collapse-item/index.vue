<template>
  <div
    :class="[
      ...
      { 'lnzi-hairline--top': index }
    ]"
  >
    <lnzi-cell :class="createClass('title')" @click="onClick" v-bind="$props">
  ...
  watch: {
    expanded (current, prev) {
      ...
      this.$nextTick(() => {
        ...
        const amimationFn = window.requestAnimationFrame || window.webkitRequestAnimationFrame
